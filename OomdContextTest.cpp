...
TEST_F(OomdContextTest, TreeWalk) {
  ...
  const CgroupNode* pp4 = ctx.getCgroupNode(p4);
  ASSERT_TRUE(pp4->parent);
  EXPECT_EQ(pp4->parent->path, p3);
  ASSERT_TRUE(pp4->parent->parent);
  EXPECT_EQ(pp4->parent->parent->path, p2);
  ASSERT_TRUE(pp4->parent->parent->parent);
  EXPECT_EQ(pp4->parent->parent->parent->path, p1);
  ASSERT_TRUE(pp4->parent->parent->parent->parent);
  EXPECT_TRUE(pp4->parent->parent->parent->parent->path.isRoot());
}
